...
/// High-res linear accumulation buffer: per cell `[r_sum, g_sum, b_sum, count]`.
struct Accum {
    w: usize,
    data: Vec<[f64; 4]>,
}
...
impl Accum {
    ...
    #[inline]
    fn add(&mut self, x: usize, y: usize, rgb: [f64; 3]) {
...
pub fn render(genome: &Genome, opts: &RenderOpts) -> Vec<u8> {
    ...
    downsample_and_tonemap(&accum, genome, opts)
}

/// Box-downsample the accumulation by `ss`, then tone-map to RGBA8.
fn downsample_and_tonemap(accum: &Accum, genome: &Genome, opts: &RenderOpts) -> Vec<u8> {
    ...
    let log_max = (1.0 + max_count).ln().max(1e-12);
    ...
    for (i, cell) in down.iter().enumerate() {
        ...
        if freq <= 0.0 {
            ...
            let l = ((1.0 + freq).ln() / log_max * genome.brightness).clamp(0.0, 1.0);
            let lg = l.powf(inv_gamma);
            // Per-channel gamma vs gamma-on-brightness, blended by vibrancy.
            let mut ch = [0.0f64; 3];
            for c in 0..3 {
                let by_brightness = avg[c] * lg;
                let by_channel = (avg[c] * l).powf(inv_gamma);
